...
(attributions, tâches restantes, etc.) qui peuvent être effectuées par des utilisateurs
ayant des permissions spécifiques sur un champ, sans être administrateur.
Chaque route vérifie l'authentification puis les permissions granulaires de l'utilisateur.
"""
...
# Toutes les routes ici commenceront par /api (ex: /api/attributions/ajouter).
...
def api_ajouter_attribution() -> Any:
    ...
    Accessible aux admins et aux utilisateurs ayant accès au champ de l'enseignant.
    """
    data = request.get_json()
    if not data or not (enseignant_id := data.get("enseignant_id")) or not (code_cours := data.get("code_cours")):
        return jsonify({"success": False, "message": "Données manquantes."}), 400

    # Étape 1: Vérifier les permissions de l'utilisateur
    champ_no_enseignant = db.get_enseignant_champ_no(enseignant_id)
    ...
    # Étape 2: Vérifier les règles métier (champ verrouillé, groupes restants)
    ...
    if db.get_groupes_restants_pour_cours(code_cours) < 1:
        return jsonify({"success": False, "message": "Plus de groupes disponibles pour ce cours."}), 409

    # Étape 3: Exécuter l'action
    nouvelle_attribution_id = db.add_attribution(enseignant_id, code_cours)
        ...
        jsonify(
            {
                ...
                "groupes_restants_cours": db.get_groupes_restants_pour_cours(code_cours),
...
def api_supprimer_attribution() -> Any:
    ...
    Accessible aux admins et aux utilisateurs ayant accès au champ de l'enseignant concerné.
    """
    data = request.get_json()
...
def api_creer_tache_restante(champ_no: str) -> Any:
    ...
    Accessible aux admins et aux utilisateurs ayant accès à ce champ.
    """
    ...
    nouveau_fictif = db.create_fictif_enseignant(champ_no)
...
def api_supprimer_enseignant(enseignant_id: int) -> Any:
    ...
    """
    # Étape 1: Récupérer les informations et vérifier les permissions
    champ_no = db.get_enseignant_champ_no(enseignant_id)
    if not champ_no:
        return jsonify({"success": False, "message": "Enseignant non trouvé."}), 404
    if not current_user.can_access_champ(champ_no):
        return jsonify({"success": False, "message": "Accès non autorisé à ce champ."}), 403
    ...
    cours_liberes_details = [{"code_cours": code, "nouveaux_groupes_restants": db.get_groupes_restants_pour_cours(code)} for code in cours_affectes]
    return jsonify(
        {
            "success": True,
