...
def get_all_champs():
    """Récupère tous les champs, y compris leur statut de verrouillage, triés par numéro."""
    db = get_db()
    if not db:
        return []
    try:
...
def get_champ_details(champ_no):
    """Récupère les détails d'un champ spécifique, y compris son statut de verrouillage."""
    db = get_db()
    if not db:
        return None
    try:
...
def get_all_enseignants_avec_details():
    """
    Récupère tous les enseignants avec les détails de leur champ (y compris le verrouillage),
    leurs périodes de cours et autres.
...
def calculer_donnees_sommaire():
    ...
    Retourne les enseignants groupés par champ avec totaux, les moyennes par champ (avec statut de verrouillage),
...
def api_ajouter_attribution():
    ...
    try:
        with db.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            # Vérifier le statut de verrouillage du champ avant toute modification
            query_verrou = """
            ...
            if verrou_info and verrou_info["estverrouille"] and not verrou_info["estfictif"]:
                ...
                return jsonify({"success": False, "message": msg}), 403  # 403 Forbidden
...
def api_creer_tache_restante(champ_no):
    """API pour créer une nouvelle tâche restante (enseignant fictif). Fonctionne même si le champ est verrouillé."""
    db = get_db()
    if not db:
        return jsonify({"success": False, "message": "Erreur de connexion à la base de données."}), 500
            ...
            jsonify(
                {
                    ...
                    "enseignant": {**nouvel_enseignant_fictif_cree, "attributions": []},
...
def api_supprimer_enseignant(enseignant_id):
    ...
    try:
        with db.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            # La FK avec ON DELETE CASCADE gère la suppression des attributions.
